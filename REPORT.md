...
|---|---|---|---|---|
| RQ-01 | 플랫폼 가입과 회원 관리 | 비공개 로그인 아이디, 고유 공개 닉네임, 로그인, 닉네임 검색, 프로필 사진·소개글, 비밀번호 변경 | `auth.py`, `users.py` | 가입, 해시, 중복, 비공개 표시, 이미지 재인코딩, 잠금, XSS 테스트 |
| RQ-02 | 상품 등록과 조회 | 상품명·가격·사진·설명, 목록·상세, 본인 상품 관리 | `products.py` | 소유권·이미지 테스트 |
| RQ-03 | 사용자 간 소통 | 상품 상세 댓글, 상품별 구매자·판매자 실시간 1:1 채팅 | `products.py`, `chat.py` | 댓글 인증·XSS·삭제 권한, 대화 고유성·IDOR·Socket 테스트 |
...
| RQ-07 | 관리자 관리 | 회원 상태·프로필 콘텐츠, 상품, 댓글, 신고, 구매, 거래, 대화·메시지, 감사 로그의 검색·페이지 이동·조치 | `admin.py` | 전체 관리 화면, 일반 사용자 403, 댓글·메시지 숨김·복구, 프로필 초기화, 구매 조회, 원장 정정, 페이지 이동 테스트 |
...
기존 `username`은 로그인 식별자와 공개 표시 이름을 동시에 담당해 로그인 아이디가 상품, 채팅, 신고, 거래 화면에 노출됐다. 이를 비공개 `login_id`와 공개 `nickname`으로 분리했다. 회원가입은 닉네임, 아이디, 비밀번호 순서로 받고 일반 사용자 화면과 Socket.IO 메시지는 닉네임만 표시한다. 관리자 화면은 신고 처리와 계정 복구를 위해 닉네임과 아이디를 함께 확인할 수 있다.

아이디는 영문 소문자, 숫자, 밑줄 4~20자로 제한한다. 닉네임은 한글, 영문, 숫자, 밑줄, 공백 2~20자를 허용하되 NFKC 정규화, 대소문자 통일, 중복 방지, 관리자 및 서비스 사칭 이름 차단을 적용한다. 사용자가 요청한 사용성을 반영해 비밀번호 최소 길이는 10자에서 8자로 조정했으며 영문, 숫자, 특수문자를 모두 포함해야 하는 기존 조합 조건과 Argon2id 저장 방식은 유지했다.

기존 SQLite의 `username` 열은 애플리케이션 시작 시 `login_id`로 변경하고 정규화 아이디, 닉네임, 프로필 이미지 파일명 열과 고유 인덱스를 추가한다. 기존 표시 이름은 최초 닉네임으로 보존하며 마이그레이션 전 DB 백업과 자동 회귀 테스트를 추가했다.
...
주요 무결성 제약은 정규화된 아이디와 닉네임 중복 금지, 잔액 0원 이상, 상품 가격 양수, 상품·구매자·판매자별 대화방 하나, 사용자별 동일 대상 신고 한 번, 멱등키별 거래 한 번, 상품별 구매 한 번이다.
...
5. 서버가 확인한 발신자의 닉네임과 메시지를 같은 방 참여자에게만 전송한다.

## 7. 기능 구현

### 회원

아이디는 영문 소문자, 숫자, 밑줄 4~20자로 제한하고 로그인에만 사용한다. 닉네임은 한글, 영문, 숫자, 밑줄, 공백 2~20자로 제한하고 NFKC 정규화 후 고유성을 검사한다. 일반 사용자에게는 닉네임만 표시하며 아이디는 관리자 계정 관리 화면에서만 확인할 수 있다. 비밀번호는 영문, 숫자, 특수문자를 포함한 8~128자로 받고 Argon2id로 저장한다. 로그인 실패가 5회 누적되면 5분 동안 계정을 잠그고 IP별 요청 제한도 함께 적용한다. 기본 세션은 30분 만료와 strong session protection을 사용하며, 사용자가 선택한 경우만 HttpOnly·SameSite 쿠키로 아이디를 기억하거나 최대 30일간 로그인 상태를 유지한다. 로그아웃하면 로그인 유지 쿠키를 즉시 삭제한다.
...
강의 요구사항의 “관리자가 플랫폼의 모든 요소를 관리”한다는 문장을 데이터 특성에 맞게 다음과 같이 분석했다. 회원은 상태 변경과 부적절한 소개글·프로필 사진 초기화, 상품은 수정과 공개 상태 조치, 댓글과 채팅 메시지는 원문을 보존한 숨김·복구, 신고는 조치·기각, 구매는 상품·참여자·거래 연결 조회, 거래는 원본을 보존한 반대 방향 정정, 감사 로그는 변경 없이 조회하도록 구현했다. 대화방은 참여자와 메시지를 함께 확인한다.

모든 관리자 목록에는 서버 검증 검색과 50개 단위 페이지 이동을 적용해 기존의 최근 500개 제한처럼 이후 데이터에 접근할 수 없는 문제를 제거했다. 일반 회원의 관리자 URL과 POST 요청은 `403`으로 거부하고 상태 변경에는 CSRF 토큰과 5~500자 관리자 사유가 필요하다. 관리자, 대상, 조치, 사유, IP와 시각은 감사 로그에 기록한다.

거래 원장, 구매 기록과 감사 로그는 임의 수정·삭제를 허용하지 않는다. 이는 관리 기능 누락이 아니라 재무 기록과 관리자 행위의 사후 추적성을 유지하기 위한 의도적인 무결성 설계다.
...
- 상품·프로필 이미지 형식 검사, 크기 제한, 재인코딩, UUID 파일명
...
- 관리자 전체 목록 검색·페이지 이동과 댓글·메시지 원문 보존형 조치
- uv 잠금 파일, Ruff, Bandit, pip-audit와 pytest 회귀 테스트

세부 결과는 `SECURITY_CHECKLIST.md`에 제공된 27개 원문 항목과 확장 19개 항목으로 기록했다.
...
68 passed, 1 warning in 25.17s
TOTAL 1482 statements / coverage 79%
...
주요 테스트는 아이디와 닉네임 분리, 닉네임 중복 및 사칭 방지, 아이디 비공개 표시, 아이디 기억·로그인 유지·로그아웃 쿠키 삭제, 8자 조합 비밀번호, 기존 사용자·메시지 DB 마이그레이션, 초기 잔액, 로그인 잠금, XSS, CSRF, 상품 소유권, 이미지 위장, 이미지 재인코딩, 이미지가 있을 때 빈 상품 설명 허용, 사진·설명 동시 누락 거부, 원화 송금, 가격 위변조, 중복 구매, 구매 경쟁, 관리자 정정, 전체 관리 화면 접근, 댓글·메시지 숨김·복구, 프로필 콘텐츠 초기화, 구매 기록 조회, 관리 목록 검색·페이지 이동, 3인 신고, 중복 신고, 채팅방 IDOR, Socket 참여자 검증, host와 port 검증, 1,000개 상품 검색 성능을 포함한다.
...
| 전체 실행에서 Socket 테스트가 간헐 실패 | 전역 Socket.IO 확장을 테스트 앱마다 다시 초기화 | 앱 fixture를 세션 범위로 두고 DB만 테스트마다 초기화 | 전체 26개 반복 실행 통과 |
| 의존성 감사에서 16건 발견 | 최초 선택 버전에 2026년 공개 취약점 존재 | 수정 버전으로 갱신 | audit 0건, pytest 26개 재통과 |
| 로그인 아이디가 모든 화면에 노출됨 | `username`을 로그인과 표시 이름으로 함께 사용 | `login_id`와 `nickname` 분리, 공개 화면은 닉네임만 표시 | 비공개 표시, 닉네임 검증, 기존 DB 마이그레이션 테스트 통과 |
...
| 유효한 CSRF 토큰으로 회원가입 시 500 오류 | CSRF 만료 시간을 라이브러리가 요구하는 초 단위 정수 대신 `timedelta`로 설정 | 만료 시간을 `7200`초로 수정하고 실제 토큰 기반 가입 테스트 추가 | HTTP 가입 `302 /login`, DB 저장 확인 후 진단 계정 삭제, pytest 36개 통과 |
| 코드 리뷰 시 로직과 보안 판단을 따라가기 어려움 | 기능 중심 구현으로 코드 가까이에 설명이 부족함 | 전체 Python 함수·클래스, JavaScript, Jinja, CSS, 테스트에 쉬운 한국어 주석을 추가하고 주석 끝 마침표를 제거 | Python AST 기준 함수·클래스 주석 누락 0개, 주석 끝 마침표 0개, pytest 68개와 보안 검사 재통과 |
| 상품 상세 화면에서 공개 질문을 남길 수 없음 | 구매 전 문의 수단이 판매자와의 비공개 채팅으로 한정됨 | 로그인 회원의 상품 댓글 작성과 본인·관리자 삭제, 비로그인 입력 비활성화 및 로그인 복귀 링크 구현 | 비로그인 차단, 저장형 XSS 이스케이프, 삭제 권한, 안전한 `next` 경로 자동 테스트 통과 |
| 내 상품 화면에 `active` 상태가 노출됨 | 화면별로 상품 상태 표시 방식이 서로 다름 | 공통 Jinja 필터로 내부 상태를 한국어로 변환 | 상품 목록·상세·내 상품에서 `판매 중` 표시, 회귀 테스트 통과 |
| 프로필에 회원을 구분할 사진이 없음 | 닉네임과 소개글만 저장하는 초기 구조 | 안전한 프로필 사진 등록·교체·삭제와 공개 표시 구현 | 재인코딩, UUID 파일명, 위장 파일 거부, 미연결 파일 404 테스트 통과 |
| 비로그인 메인 화면에서 상품으로 바로 이동하기 어려움 | 시작 버튼이 회원가입과 로그인만 제공됨 | `상품 둘러보기` 공개 목록 버튼 추가 | 비로그인 메인 화면의 `/products` 링크 자동 테스트 통과 |
| 내 상품 화면에서 업로드 사진을 확인할 수 없음 | 공개 상품 목록에만 이미지 카드 구조를 적용함 | 내 상품 카드에도 4:3 지연 로딩 미리보기와 사진 없음·삭제 상품 대체 표시 적용 | 재인코딩된 이미지 URL이 상품 목록과 내 상품 화면에 모두 표시되는지 자동 테스트 통과 |
| 대화 목록에서 문의 중인 상품을 사진으로 구분할 수 없음 | 대화방 제목과 참여자만 표시함 | 공개 가능한 상품의 4:3 지연 로딩 미리보기를 추가하고 숨김·삭제 상품은 사진 비공개 처리 | 대화 참여자 화면에 상품 이미지 URL과 지연 로딩 속성이 표시되는지 자동 테스트 통과 |
| 코드와 템플릿 수정마다 개발 서버를 수동 재시작해야 함 | 디버그 모드를 모든 환경에서 고정 비활성화함 | 개발 환경만 디버그·Python 리로더·Jinja 자동 갱신·정적 파일 캐시 해제를 적용하고 운영 환경은 모두 비활성화 | 개발·운영 설정값과 잘못된 환경 이름 거부 테스트 통과, 전체 회귀 테스트 통과 |
| 관리자가 플랫폼의 모든 요소를 관리한다는 요구사항을 엄격히 충족하지 못함 | 댓글·메시지·프로필 콘텐츠·구매 전용 관리가 없고 각 목록이 최근 500개에서 잘림 | 요소별 관리 화면, 댓글·메시지 숨김·복구, 프로필 초기화, 구매 조회, 전체 목록 검색·페이지 이동, 기존 메시지 상태 마이그레이션 구현 | 전체 관리 화면 200, 일반 사용자 403, 조치·감사 로그·원문 보존·페이지 이동·검색 자동 테스트 통과 |
| 페이지마다 상단 메뉴를 따로 관리할 가능성이 있음 | 공통 상단 바 코드가 `base.html`의 다른 HTML 구조와 섞여 있음 | 로그인·일반 회원·관리자 조건을 보존한 `partials/topbar.html`로 분리하고 `base.html`에서 include | 공개 화면 4개, 회원 화면 4개, 관리자 화면 3개에서 상단 바 1개와 상태별 메뉴 자동 테스트 통과 |
| 목록 화면의 영어 보조 제목·한글 제목·선택 설명·작업 버튼 구조가 반복됨 | 각 화면이 `page-head` HTML을 직접 작성함 | 선택 설명과 `call` 작업 영역을 지원하는 `components/page_header.html` 매크로로 분리하고 상품 목록·내 상품·회원 찾기에 적용 | 비로그인·로그인 상태별 제목, 설명, 상품 등록 버튼과 페이지 헤더 1개 자동 테스트 통과 |
| 상품 목록마다 이미지·상태·상품명·가격 카드 HTML이 반복됨 | 공개 목록, 내 상품, 대화 목록이 각자 같은 카드 구조를 작성함 | `components/product_card.html`로 통합하고 내 상품의 수정 버튼만 `call` 작업 슬롯으로 전달 | 세 목록에서 같은 카드·상품·가격 출력, 내 상품에만 수정 버튼 표시, 이미지 미리보기 회귀 테스트 통과 |
| 거래·신고 내역의 반응형 표와 빈 상태 행이 반복됨 | 화면마다 `table-wrap`, 열 제목, 반복문과 `colspan`을 직접 작성함 | 열 제목·행 목록·빈 문구를 받는 `components/data_table.html`로 통합하고 열 개수로 `colspan` 자동 계산 | 거래 6열·신고 5열 제목과 빈 상태 문구·colspan 자동 테스트 통과 |
...
README에 uv, Docker Compose, 관리자 생성, ngrok, 환경변수, 테스트 명령을 정리했다. 로컬 환경과 Docker는 모두 `pyproject.toml`과 `uv.lock`을 기준으로 설치한다. 개발 서버 주소는 명령행 옵션, `.env` 또는 운영체제 환경변수, 안전한 기본값 순으로 적용하고 host와 port를 실행 전에 검증한다. 환경변수가 없어도 첫 실행 시 `instance/.secret_key`에 무작위 비밀키를 만들기 때문에 저장소를 복제한 뒤 별도 하드코딩 비밀키 없이 실행할 수 있다.
...
7개 필수 요구사항을 Flask 웹앱으로 구현했고 기존 예제의 평문 비밀번호, 하드코딩 비밀키, CSRF 부재, 권한 검사 부족, 로그인 아이디 공개, 사용자 사칭 전체 채팅, 검증 없는 신고, debug 실행 문제를 제거했다. 관리자는 회원·프로필 콘텐츠·상품·댓글·신고·구매·거래·대화·메시지·감사 로그 전체를 데이터 특성에 맞는 안전한 방식으로 관리할 수 있다. 68개 자동 테스트, 정적 분석, 의존성 감사로 주요 동작과 보안 통제를 확인했다.
